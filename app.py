import streamlit as st
import pandas as pd
import yfinance as yf
import pandas_ta as ta
from datetime import datetime

# --- 1. SETTING HALAMAN ---
st.set_page_config(page_title="Infectious Actio Clone", layout="wide")

# --- 2. DATABASE SAHAM IDX ---
TICKERS = ["BBCA.JK", "BBRI.JK", "BMRI.JK", "TLKM.JK", "ASII.JK", "GOTO.JK", "BBNI.JK", "ADRO.JK", "UNVR.JK", "ANTM.JK"]

# --- 3. DATA ENGINE (Anti-Limit & Anti-Crash) ---
@st.cache_data(ttl=3600)
def load_idx_data(tickers):
    try:
        # Download massal agar efisien
        data = yf.download(tickers, period="6mo", interval="1d", group_by='ticker', progress=False)
        return data
    except Exception:
        return None

# --- 4. TAMPILAN DASHBOARD ---
st.title("📈 Infectious Actio Scanner V.3")
st.caption(f"Sinkronisasi Terakhir: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

tab1, tab2, tab3 = st.tabs(["🕒 Day Scalping", "📅 Weekly Swing", "🏛️ Monthly Invest"])

raw_data = load_idx_data(TICKERS)

def process_screener(label):
    if raw_data is None or raw_data.empty:
        st.error("Gagal memuat data. Server sedang sibuk (Rate Limit).")
        return

    rows = []
    for t in TICKERS:
        try:
            df_t = raw_data[t].dropna()
            if len(df_t) < 30: continue
            
            # Kalkulasi Indikator
            df_t['RSI'] = ta.rsi(df_t['Close'], length=14)
            bb = ta.bbands(df_t['Close'], length=20, std=2)
            
            last_row = df_t.iloc[-1]
            last_bb = bb.iloc[-1]
            
            price_val = float(last_row['Close'])
            rsi_val = float(last_row['RSI'])
            
            # Deteksi kolom BBL/BBU secara otomatis
            bbl_col = [c for c in bb.columns if c.startswith('BBL')]
            u_col = [c for c in bb.columns if c.startswith('BBU')]
            l_band = float(last_bb[bbl_col])
            u_band = float(last_bb[u_col])
            
            # Logic Sinyal
            signal = "HOLD"
            if price_val <= l_band or rsi_val < 35:
                signal = "BUY"
            elif price_val >= u_band or rsi_val > 70:
                signal = "SELL"
                
            rows.append({
                "SAHAM": t.replace(".JK", ""),
                "HARGA": int(price_val),
                "SINYAL": signal,
                "RSI": round(rsi_val, 2),
                "ZONE": "OVERSOLD" if rsi_val < 35 else "OVERBOUGHT" if rsi_val > 70 else "NEUTRAL"
            })
        except:
            continue

    if rows:
        df_display = pd.DataFrame(rows)
        # Warna Sinyal
        def style_sig(val):
            if val == 'BUY': return 'color: #00ff00; font-weight: bold'
            if val == 'SELL': return 'color: #ff4b4b; font-weight: bold'
            return ''
        
        st.dataframe(df_display.style.map(style_sig, subset=['SINYAL']), width="stretch", hide_index=True)
    else:
        st.warning("Data belum tersedia saat ini.")

# Jalankan Tab
with tab1: process_screener("Daily")
with tab2: process_screener("Weekly")
with tab3: process_screener("Monthly")
