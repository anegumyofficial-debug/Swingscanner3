...
# CSS untuk Bar Metrik (Metrics) agar kelihatan profesional
st.markdown("""
    <style>
...
st.write(f"Kemas Kini Terakhir: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} WIB")
st.markdown("---")

# 2. SENARAI SAHAM (TICKERS)
tickers = ["BBRI.JK", "BBCA.JK", "BBNI.JK", "ASII.JK", "TLKM.JK", "BMRI.JK"]

# 3. FUNGSI ANALISIS (LOGIKA DATA TERKINI & ANTI-KOSONG)
def fetch_and_analyze(ticker, timeframe_label):
    ...
    # Ambil data dengan tempoh lebih lama supaya sentiasa ada data sandaran
    ...
    # AMBIL DATA TERAKHIR YANG SAH (Mencari harga terakhir jika pasaran tutup)
